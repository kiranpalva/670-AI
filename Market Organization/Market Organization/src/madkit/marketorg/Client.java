...
public class Client extends Agent {
    ...
    protected void activate() {
		...
		requestRole(MarketOrganization.COMMUNITY,
					MarketOrganization.CLIENT_GROUP,
					MarketOrganization.CLIENT_ROLE, null);
		int pause = 1000 + (int) (Math.random() * 2000);
		getLogger().info(() -> "I will be looking for a " + product + " in " +
				pause + " ms !");
		pause(pause);
    }
    ...
    @Override
    protected void end() {
		getLogger().info(() -> "I will quit soon now, but I will launch another" +
				" one like me !");
    ...
    private boolean buyTicket(StringMessage brokerAnswer) {
		...
		requestRole(MarketOrganization.COMMUNITY, contractGroupID,
					MarketOrganization.CLIENT_ROLE);
		Message ticket = sendMessageAndWaitForReply(
				MarketOrganization.COMMUNITY, contractGroupID,
				MarketOrganization.PROVIDER_ROLE, new StringMessage("money"), 4000);
    ...
    public void setupFrame(AgentFrame frame) {
		...
		if (xLocation + 390 > Toolkit.getDefaultToolkit().getScreenSize()
                                     .getWidth()) {
    ...
    public static void main(String[] args) {
		...
		new Madkit(Option.launchAgents.toString(),
		   Broker.class.getName() + ",true,3;" + Client.class.getName() +
				   ",true,2;" + Provider.class.getName() + ",true,7");
		
//		new Madkit(Option.launchAgents.toString(),
//				   Broker.class.getName() + ",true,1;" +
//						   Client.class.getName() + ",true,1;" +
//						   Provider.class.getName() + ",true,3");
    }
}